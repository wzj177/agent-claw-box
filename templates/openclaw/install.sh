...
# ---------------------------------------------------------------------------
# CN_NETWORK: "" = not probed yet, "1" = domestic/CN, "0" = open Internet
CN_NETWORK=""
DETECTED_CN_REGISTRY="https://registry.npmmirror.com"
DETECTED_CN_SHARP_HOST="https://npmmirror.com/mirrors/sharp/"
DETECTED_CN_NODE_MIRROR="https://npmmirror.com/mirrors/node/"

detect_cn_network() {
    if [[ -n "$CN_NETWORK" ]]; then
        return 0
    fi
    local ok=false
    if command -v curl &>/dev/null; then
        if curl -fsSL --max-time 5 --connect-timeout 5 --proto '=https' --tlsv1.2 \
                "https://registry.npmjs.org/-/ping" -o /dev/null 2>/dev/null; then
            ok=true
        fi
    elif command -v wget &>/dev/null; then
        if wget -q --timeout=5 --tries=1 \
                "https://registry.npmjs.org/-/ping" -O /dev/null 2>/dev/null; then
            ok=true
        fi
    fi
    if [[ "$ok" == "true" ]]; then
        ...
        CN_NETWORK=1
        ui_info "CN network detected: switching to domestic mirrors (npmmirror.com)"
    fi
}

is_cn_network() {
    if [[ -z "$CN_NETWORK" ]]; then
        detect_cn_network
    fi
    [[ "${CN_NETWORK}" == "1" ]]
}
...
run_npm_global_install() {
    ...
    # CN-aware: inject domestic npm registry and sharp binary host when npmjs.org is unreachable
    local -a npm_extra_flags=()
    local -a env_extras=("SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS")
    if is_cn_network; then
        npm_extra_flags+=(--registry "${DETECTED_CN_REGISTRY}")
        env_extras+=("npm_config_sharp_binary_host=${DETECTED_CN_SHARP_HOST}")
    fi

    local -a cmd
    cmd=(env "${env_extras[@]}" npm --loglevel "$NPM_LOGLEVEL")
    if [[ -n "$NPM_SILENT_FLAG" ]]; then
        cmd+=("$NPM_SILENT_FLAG")
    fi
    cmd+=(--no-fund --no-audit install -g "${npm_extra_flags[@]}" "$spec")
    local cmd_display=""
...
ensure_pnpm() {
    ...
    fix_npm_permissions
    local -a pnpm_install_cmd=(npm install -g pnpm@10)
    if is_cn_network; then
        pnpm_install_cmd+=(--registry "${DETECTED_CN_REGISTRY}")
    fi
    run_quiet_step "Installing pnpm" "${pnpm_install_cmd[@]}"
    refresh_shell_command_cache
    if detect_pnpm_cmd && pnpm_cmd_is_ready; then
        ui_success "pnpm ready ($(pnpm_cmd_pretty))"
        return 0
...
resolve_beta_version() {
    local -a npm_reg_arg=()
    if is_cn_network; then
        npm_reg_arg=(--registry "${DETECTED_CN_REGISTRY}")
    fi
    local beta=""
    beta="$(npm view "${npm_reg_arg[@]}" openclaw dist-tags.beta 2>/dev/null || true)"
...
install_openclaw() {
    ...
    local -a npm_view_reg_arg=()
    if is_cn_network; then
        npm_view_reg_arg=(--registry "${DETECTED_CN_REGISTRY}")
    fi
    local resolved_version=""
    resolved_version="$(npm view "${npm_view_reg_arg[@]}" "${package_name}@${OPENCLAW_VERSION}" version 2>/dev/null || true)"
